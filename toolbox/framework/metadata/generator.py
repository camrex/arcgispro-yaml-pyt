...
This script reads the documentation section from tool YAML files and generates
corresponding .pyt.xml metadata files that ArcGIS Pro can display.
"""
...
def wrap_in_html_div(text: str, preserve_newlines: bool = False) -> str:
    ...
    for line in lines:
        ...
        if (
            ...
            # Add as new line (either new bullet or regular text line)
...
def generate_metadata_for_tool(
    ...
    print(f"Generated metadata: {xml_path}")
...
def generate_all_metadata() -> None:
    """Generate metadata XML for all enabled tools in the toolbox."""
    ...
    # Output directory (same as .pyt file)
    output_dir = Path(__file__).parent.parent

    print("Generating tool metadata XML files...")
    print(f"Output directory: {output_dir}")
    print()

    # Generate toolbox metadata
    if toolbox_config.documentation:
        print("Generating toolbox metadata...")
        toolbox_xml_content = create_toolbox_metadata_xml(toolbox_config)
        toolbox_xml_path = output_dir / "yaml_toolbox.pyt.xml"

        with open(toolbox_xml_path, "w", encoding="utf-8") as f:
            f.write(toolbox_xml_content)

        print(f"✓ Toolbox metadata: {toolbox_xml_path}")
        print()

    # Process each enabled tool
    for tool_ref in toolbox_config.tools:
        if not tool_ref.enabled:
            print(f"Skipping disabled tool: {tool_ref.name}")
            continue

        # Construct tool config path
        ...
        # Convert tool name to class name (e.g., buffer_analysis -> BufferAnalysisTool)
        class_name = "".join(word.capitalize() for word in tool_ref.name.split("_")) + "Tool"

        # Generate metadata
        try:
            generate_metadata_for_tool(tool_config_path, output_dir, class_name)
        except Exception as e:
            print(f"Error generating metadata for {tool_ref.name}: {e}")
            continue
    ...
    print()
    print("Next steps:")
    print("1. Review the generated .pyt.xml files")
    print("2. Open the toolbox in ArcGIS Pro")
    print("3. Right-click a tool > Item Description to view/edit metadata")
    print("4. Compare the XML structure with what ArcGIS Pro generates")
    print("5. Adjust the XML generation code as needed")
